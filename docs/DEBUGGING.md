...
| `?date=MM-DD`, `?date=YYYY-MM-DD` | Loads that recurring calendar day, or an exact-date editorial edition when one exists, instead of today's facts. The exact form is how a one-day edition is checked before it goes live. `--date` on the browser tools. | `lib/daily-facts.ts` |
