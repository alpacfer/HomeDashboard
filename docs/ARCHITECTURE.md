...
│   ├── DAILY_SOURCES,             lib/daily-forecast.ts  Google days, then Open-Meteo daily
│   │   parseGoogleDaily,                                aggregates, then MET Norway aggregated by
│   │   parseDailyForecast,                              day; the seven days after today, day
│   │   parseMetDaily, describeDay                       condition
...
`WeatherPanel` fetches the forecast every 15 minutes, on reconnect/visibility resume, or when the user presses Enter or `R`. `lib/forecast-sources.ts` owns the providers and is asked in preference order: **Google first, DMI Open Data second, Open-Meteo third, MET Norway fourth**. The last three are keyless and CORS-open and are fetched straight from the browser; Google is the one that needs a route.

**Google is WeatherNext 3**, the model Google DeepMind published on 3 September 2026 and began serving through the Maps Platform Weather API the same day. It is a 5 km global model initialised hourly, and it leads on the precipitation skill Google claims for it rather than on resolution: the Harmonie run behind it is a 2 km national model, which on a Danish location is the sharper of the two. That is why the three providers below it are unchanged.

It is the only provider here that cannot be fetched from the browser. `weather.googleapis.com` does answer cross-origin, but the key would have to travel with the request, and `scripts/check-rules.mjs` refuses `NEXT_PUBLIC_*KEY*` because a key inlined into the bundle is a key any viewer can spend. So `app/api/weather/route.ts` holds `GOOGLE_WEATHER_API_KEY` and the browser asks it, exactly as `/api/departures` holds `REJSEPLANEN_ACCESS_ID`; the display keeps that route awake every two minutes anyway, so this costs no extra wake-up. The route decides the location as well as the key — it ignores anything a caller says about where — so the endpoint cannot be turned into an open proxy onto a metered key.

The request is shaped by the free tier. `hours` caps `pageSize` at 24 and each page is a billable call, so asking for the 72 hours the other providers hand back in one response would cost three calls a refresh. The card only draws `RIBBON_HOURS` of them (18), so one 24-hour page is asked for and the display spends about 2,920 calls a month on the card and 730 on the week, against 10,000 free. The week asks for the `days` maximum of 10 rather than the 8 it draws, because **a Google day is not a calendar day**: a `ForecastDay` runs 07:00 to 07:00 local, so between Copenhagen midnight and 07:00 the first day returned still carries yesterday's date and `parseGoogleDaily()` drops two days rather than one. At 8 days that left six, one short of a week, and the strip fell through to Open-Meteo for seven hours every night while still spending a Google call an hour to discover it. Ten days cost the same single call. The 07:00 boundary also means a Google day and an Open-Meteo day are not quite the same day; the two are never mixed in one row, and the highs and lows land within a degree of the other providers. `lib/google-weather.ts` documents the arithmetic and the units: every measurement arrives beside the name of its unit and is checked rather than trusted, so a response that switched to Fahrenheit is refused instead of drawn as a mild afternoon. The route caches each forecast for less than the refresh interval of the panel that asks, which costs the display no freshness and absorbs the burst of page loads from `npm run audit` and `npm run shot`.

Behind Google there are still three, because there must always be a fallback and one of them must not share a failure mode with the others — and because Google is the only metered provider here, the keyless chain has to be able to carry the display on its own. DMI's forecast EDR enforces a fair-use limit of 500 requests per 5 seconds shared across every caller and answers `429` when busy; during their supercomputer maintenance it answered `429` to everything for hours and the display had no forecast at all. Open-Meteo's `dmi_seamless` is the same DMI Harmonie run, checked field by field against a direct DMI capture (cloud 0.82/0.99 against 82/99, visibility 5969/5870 against 5960/5880, temperatures identical), so the fallback costs directness and nothing else. A provider that fails is skipped for an hour, so a long outage does not spend a request every refresh. See [DEPLOYMENT.md](DEPLOYMENT.md) for the limits.
...
`nextRotation()` takes a third argument saying whether the map is worth its thirty seconds, and skips the step into it when it is not. `ForecastMapPanel` decides and reports it through `onDry`: a loaded run, frames still ahead of now, and no precipitation anywhere on the grid within the six hours it would animate. Loading, a failed provider and an expired run each keep the scene, because each has something honest to say. The panel stays mounted and its refresh scheduler keeps running while the scene is skipped, so the map rejoins the rotation by itself when a later run brings rain — no reload, and no extra request for having been away. Two consequences are deliberate: the report is held back until the scene has been on screen once, because that first appearance is what measures the view and arms the scheduler, and a map already on screen when a refresh turns the forecast dry plays out its scene rather than being cut off. `RotatingPanel` reads the flag through a ref, so a forecast arriving mid-scene cannot restart the cycle.
